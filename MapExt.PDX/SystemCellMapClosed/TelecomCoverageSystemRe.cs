...
using static Game.Simulation.TelecomCoverageSystem;
    ...
    public struct TelecomCoverageJob : IJob
    {
        ...
        private void AddNetworkCapacity(NativeArray<CellDensityData> densityData, NativeArray<CellFacilityData> facilityData, NativeList<float> signalStrengths, ArchetypeChunk chunk, ref int arrayIndex, ref TelecomStatus status, DynamicBuffer<CityModifier> cityModifiers)
        {
            ...
            for (int i = 0; i < nativeArray.Length; i++)
            {
                ...
                {
                    UpgradeUtils.CombineStats(ref data, bufferAccessor2[i], ref this.m_PrefabRefData, ref this.m_PrefabTelecomFacilityData);
                }
                ...
                {
                    int2 @int = math.max(CellMapSystem<TelecomCoverage>.GetCell(transform.m_Position - data.m_Range, CellMapSystem<TelecomCoverage>.kMapSize, 128), 0);
                    int2 int2 = math.min(CellMapSystem<TelecomCoverage>.GetCell(transform.m_Position + data.m_Range, CellMapSystem<TelecomCoverage>.kMapSize, 128) + 1, 128);
                    int2 int3 = int2 - @int;
                    if (!math.any(int3 <= 0))
                    {
        ...
        private void CalculateSignalStrength(NativeArray<CellFacilityData> facilityData, NativeArray<float> obstructSlopes, NativeList<float> signalStrengths, ArchetypeChunk chunk, DynamicBuffer<CityModifier> cityModifiers)
        {
            ...
            for (int i = 0; i < nativeArray.Length; i++)
            {
                ...
                int2 @int = math.max(CellMapSystem<TelecomCoverage>.GetCell(position - data.m_Range, CellMapSystem<TelecomCoverage>.kMapSize, 128), 0);
                int2 int2 = math.min(CellMapSystem<TelecomCoverage>.GetCell(position + data.m_Range, CellMapSystem<TelecomCoverage>.kMapSize, 128) + 1, 128);
                int2 int3 = int2 - @int;
                if (math.any(int3 <= 0))
                {
                    continue;
                ...
                {
                    this.CalculateSignalStrength(subArray, @int, int2, data.m_Range, position);
                }
                else
                {
                    ...
                    this.CalculateCellSignalStrength(obstructSlopes, subArray, int4, @int, int2, data.m_Range, position);
                    int2 int5 = int4;
                    int2 int6 = int4 + 1;
                    while (math.any((int5 > @int) | (int6 < int2)))
                    {
                        if (int5.y > @int.y)
                        {
                            ...
                            {
                                this.CalculateCellSignalStrength(obstructSlopes, subArray, new int2(j, int5.y), @int, int2, data.m_Range, position);
                            }
                            for (int num = int4.x - 1; num >= int5.x; num--)
                            {
                                this.CalculateCellSignalStrength(obstructSlopes, subArray, new int2(num, int5.y), @int, int2, data.m_Range, position);
                            }
                        }
                        if (int6.y < int2.y)
                        {
                            ...
                            {
                                this.CalculateCellSignalStrength(obstructSlopes, subArray, new int2(k, int6.y), @int, int2, data.m_Range, position);
                            }
                            for (int num2 = int4.x - 1; num2 >= int5.x; num2--)
                            {
                                this.CalculateCellSignalStrength(obstructSlopes, subArray, new int2(num2, int6.y), @int, int2, data.m_Range, position);
                            }
                            int6.y++;
                        }
                        if (int5.x > @int.x)
                        {
                            ...
                            {
                                this.CalculateCellSignalStrength(obstructSlopes, subArray, new int2(int5.x, l), @int, int2, data.m_Range, position);
                            }
                            for (int num3 = int4.y - 1; num3 >= int5.y; num3--)
                            {
                                this.CalculateCellSignalStrength(obstructSlopes, subArray, new int2(int5.x, num3), @int, int2, data.m_Range, position);
                            }
                        }
                        if (int6.x < int2.x)
                        {
                            ...
                            {
                                this.CalculateCellSignalStrength(obstructSlopes, subArray, new int2(int6.x, m), @int, int2, data.m_Range, position);
                            }
                            for (int num4 = int4.y - 1; num4 >= int5.y; num4--)
                            {
                                this.CalculateCellSignalStrength(obstructSlopes, subArray, new int2(int6.x, num4), @int, int2, data.m_Range, position);
                            }
                            int6.x++;
                        }
                    }
