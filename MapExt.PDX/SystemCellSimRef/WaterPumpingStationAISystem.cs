...
{
	//[CompilerGenerated]
	public partial class WaterPumpingStationAISystem : GameSystemBase
	{
		[BurstCompile]
		public struct PumpTickJob : IJobChunk
		{
			...
			public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
			{
				...
				for (int i = 0; i < chunk.Count; i++)
				{
					...
					WaterPumpingStationData result = this.m_PumpDatas[prefab];
					if (bufferAccessor2.Length != 0)
					{
						UpgradeUtils.CombineStats(ref result, bufferAccessor2[i], ref this.m_Prefabs, ref this.m_PumpDatas);
					}
					...
					if (result.m_Types != 0)
					{
						if ((result.m_Types & AllowedWaterTypes.Groundwater) != 0)
						{
							...
							float num7 = math.clamp(num6 * (float)result.m_Capacity, 0f, (float)result.m_Capacity - num3);
							num3 += num7;
							num4 += num5 * num7;
						...
						if ((result.m_Types & AllowedWaterTypes.SurfaceWater) != 0 && bufferAccessor.Length != 0)
						{
							...
							for (int j = 0; j < dynamicBuffer.Length; j++)
							{
								...
								{
									float surfaceWaterAvailability = WaterPumpingStationAISystem.GetSurfaceWaterAvailability(componentData2.m_Position, result.m_Types, this.m_WaterSurfaceData, this.m_Parameters.m_SurfaceWaterPumpEffectiveDepth);
									float num10 = WaterUtils.SamplePolluted(ref this.m_WaterSurfaceData, componentData2.m_Position);
									float num11 = math.clamp(surfaceWaterAvailability * (float)result.m_Capacity, 0f, (float)result.m_Capacity - num3);
									num3 += num11;
									num4 += num11 * num10;
					...
					{
						num3 = result.m_Capacity;
						num4 = 0f;
						num = 0f;
					}
					reference.m_Capacity = (int)math.round(efficiency * num3 + (float)num2);
					reference.m_Pollution = ((reference.m_Capacity > 0) ? ((1f - result.m_Purification) * num4 / (float)reference.m_Capacity) : 0f);
					...
					if (bufferAccessor4.Length != 0)
					{
						...
						{
							float num12 = (num3 + (float)num2) / (float)(result.m_Capacity + num2);
							factors[19] = num12;
						}
						BuildingUtils.SetEfficiencyFactors(bufferAccessor4[i], factors);
					}
					bool flag3 = num3 < 0.1f * (float)result.m_Capacity;
		...
		private struct TypeHandle
		{
			...
			public ComponentLookup<Game.Simulation.WaterSourceData> __Game_Simulation_WaterSourceData_RW_ComponentLookup;
			...
			public void __AssignHandles(ref SystemState state)
			{
				...
				this.__Game_Simulation_WaterSourceData_RW_ComponentLookup = state.GetComponentLookup<Game.Simulation.WaterSourceData>();
