...
{
	//[CompilerGenerated]
	public partial class WaterDangerSystem : GameSystemBase
	{
		[BurstCompile]
		...
		private struct TypeHandle
		{
			...
			public void __AssignHandles(ref SystemState state)
			{
				...
				this.__Game_Buildings_EmergencyShelter_RO_ComponentLookup = state.GetComponentLookup<Game.Buildings.EmergencyShelter>(isReadOnly: true);
		...
		protected override void OnCreate()
		{
			...
			this.m_ObjectSearchSystem = base.World.GetOrCreateSystemManaged<SearchSystem>();
			this.m_EndFrameBarrier = base.World.GetOrCreateSystemManaged<EndFrameBarrier>();
			this.m_WaterLevelChangeQuery = base.GetEntityQuery(ComponentType.ReadOnly<WaterLevelChange>(), ComponentType.Exclude<Deleted>(), ComponentType.Exclude<Temp>());
			this.m_EndangerArchetype = base.EntityManager.CreateArchetype(ComponentType.ReadWrite<Game.Common.Event>(), ComponentType.ReadWrite<Endanger>());
			base.RequireForUpdate(this.m_WaterLevelChangeQuery);
		}
