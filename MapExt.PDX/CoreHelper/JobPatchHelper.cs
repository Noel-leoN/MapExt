...
namespace MapExtPDX
{
    public class JobPatchHelper
    {
        ...
        public static void ApplyAllPatches(Harmony harmonyInstance)
        {
            ...
            if (harmonyInstance == null) { Mod.Logger.Error($"[JobPatcher]错误！ Harmony实例未找到！"); return; }

            var targets = JobPatchTarget.GetPatchTargets();
            Mod.Logger.Info($"[JobPatcher] Processing {targets.Count} raw job patch target definitions...");
            // --- Stage 1: Resolve Types and Group by Method ---
            var resolvedAndGrouped = targets
                .Select(t => { // Attempt to resolve types and method for each target
                    ...
                    try
                    {
                        ...
                        if (!isValid)
                        {
                            Mod.Logger.Error($"[JobPatcher]错误！ Failed to resolve components for target: {t.TargetTypeName}.{t.TargetMethodName} -> {t.OriginalJobFullName}");
                        }
                    }
                    catch (Exception ex)
                    {
                        Mod.Logger.Error($"[JobPatcher]错误！ Exception resolving target {t.TargetTypeName}.{t.TargetMethodName}: {ex.Message}");
                        isValid = false;
                    }
            ...
            Mod.Logger.Info($"[JobPatcher] Found {resolvedAndGrouped.Count()} unique target method(s) to patch.");
            ...
            foreach (var methodGroup in resolvedAndGrouped)
            {
                ...
                Mod.Logger.Info($"[JobPatcher] --> Processing method: {targetMethod.DeclaringType.FullName}.{targetMethod.Name}");
                ...
                try
                {
                    ...
                    Mod.Logger.Info($"    Success: Queued single patch application for this method.");
                }
                catch (Exception ex)
                {
                    Mod.Logger.Error($"    错误！Fatal Error applying Harmony patch to method {targetMethod.DeclaringType.FullName}.{targetMethod.Name}: {ex}");
                    // Consider cleanup or alternative actions if patching fails here
                }
            }

            Mod.Logger.Info("[JobPatcher] Finished processing and applying patches.");
        }
