...
namespace MapExt.Systems
{
    ...
    public partial class TelecomCoverageSystem : CellMapSystem<TelecomCoverage>, IJobSerializable
    {
        ...
        public struct TelecomCoverageJob : IJob
        {
            ...
            private void AddNetworkCapacity(NativeArray<CellDensityData> densityData, NativeArray<CellFacilityData> facilityData, NativeList<float> signalStrengths, ArchetypeChunk chunk, ref int arrayIndex, ref TelecomStatus status, DynamicBuffer<CityModifier> cityModifiers)
            {
                ...
                for (int i = 0; i < nativeArray.Length; i++)
                {
                    ...
                    {
                        UpgradeUtils.CombineStats(ref result, bufferAccessor2[i], ref this.m_PrefabRefData, ref this.m_PrefabTelecomFacilityData);
                    }
                    ...
                    if (!(result.m_Range < 1f) && !(result.m_NetworkCapacity < 1f))
                    {
                        int2 @int = math.max(CellMapSystemRe.GetCell(transform.m_Position - result.m_Range, CellMapSystemRe.kMapSize, 128), 0);
                        int2 int2 = math.min(CellMapSystemRe.GetCell(transform.m_Position + result.m_Range, CellMapSystemRe.kMapSize, 128) + 1, 128);
                        int2 int3 = int2 - @int;
                        if (!math.any(int3 <= 0))
                        {
                            ...
                            float capacity = result.m_NetworkCapacity / math.max(1f, num);
            ...
            private void CalculateSignalStrength(NativeArray<CellFacilityData> facilityData, NativeArray<float> obstructSlopes, NativeList<float> signalStrengths, ArchetypeChunk chunk, DynamicBuffer<CityModifier> cityModifiers)
            {
                ...
                for (int i = 0; i < nativeArray.Length; i++)
                {
                    ...
                    int2 @int = math.max(CellMapSystemRe.GetCell(position - result.m_Range, CellMapSystemRe.kMapSize, 128), 0);
                    int2 int2 = math.min(CellMapSystemRe.GetCell(position + result.m_Range, CellMapSystemRe.kMapSize, 128) + 1, 128);
                    int2 int3 = int2 - @int;
                    if (math.any(int3 <= 0))
                    {
                        continue;
                    ...
                    {
                        this.CalculateSignalStrength(subArray, @int, int2, result.m_Range, position);
                    }
                    else
                    {
                        this.ResetObstructAngles(obstructSlopes, @int, int2);
                        int2 int4 = math.clamp(CellMapSystemRe.GetCell(position, CellMapSystemRe.kMapSize, 128), 0, 127);
                        this.CalculateCellSignalStrength(obstructSlopes, subArray, int4, @int, int2, result.m_Range, position);
                        int2 int5 = int4;
                        int2 int6 = int4 + 1;
                        while (math.any((int5 > @int) | (int6 < int2)))
                        {
                            if (int5.y > @int.y)
                            {
                                ...
                                {
                                    this.CalculateCellSignalStrength(obstructSlopes, subArray, new int2(j, int5.y), @int, int2, result.m_Range, position);
                                }
                                for (int num = int4.x - 1; num >= int5.x; num--)
                                {
                                    this.CalculateCellSignalStrength(obstructSlopes, subArray, new int2(num, int5.y), @int, int2, result.m_Range, position);
                                }
                            }
                            if (int6.y < int2.y)
                            {
                                ...
                                {
                                    this.CalculateCellSignalStrength(obstructSlopes, subArray, new int2(k, int6.y), @int, int2, result.m_Range, position);
                                }
                                for (int num2 = int4.x - 1; num2 >= int5.x; num2--)
                                {
                                    this.CalculateCellSignalStrength(obstructSlopes, subArray, new int2(num2, int6.y), @int, int2, result.m_Range, position);
                                }
                                int6.y++;
                            }
                            if (int5.x > @int.x)
                            {
                                ...
                                {
                                    this.CalculateCellSignalStrength(obstructSlopes, subArray, new int2(int5.x, l), @int, int2, result.m_Range, position);
                                }
                                for (int num3 = int4.y - 1; num3 >= int5.y; num3--)
                                {
                                    this.CalculateCellSignalStrength(obstructSlopes, subArray, new int2(int5.x, num3), @int, int2, result.m_Range, position);
                                }
                            }
                            if (int6.x < int2.x)
                            {
                                ...
                                {
                                    this.CalculateCellSignalStrength(obstructSlopes, subArray, new int2(int6.x, m), @int, int2, result.m_Range, position);
                                }
                                for (int num4 = int4.y - 1; num4 >= int5.y; num4--)
                                {
                                    this.CalculateCellSignalStrength(obstructSlopes, subArray, new int2(int6.x, num4), @int, int2, result.m_Range, position);
                                }
                                int6.x++;
                            }
                        }
            ...
            private float GetEfficiencyFactor(NativeArray<Game.Buildings.TelecomFacility> telecomFacilities, NativeArray<Temp> temps, BufferAccessor<Efficiency> efficiencyAccessor, int i)
            {
                ...
                if (temps.Length != 0)
                {
                    ...
                    if (this.m_BuildingEfficiencyData.TryGetBuffer(temp.m_Original, out var bufferData))
                    {
                        ...
                        {
                            result = TelecomCoverageSystem.GetEfficiency(bufferData);
                        }
                    }
                }
                else if (efficiencyAccessor.Length != 0)
                {
                    ...
                    {
                        result = TelecomCoverageSystem.GetEfficiency(efficiencyAccessor[i]);
                    }
                }
                return result;
            }
            ...
            {
                int2 @int = math.clamp(CellMapSystemRe.GetCell(position, CellMapSystemRe.kMapSize, 128), 0, 127);
        ...
        protected override void OnCreate()
        {
            ...
            this.m_FacilityQuery = base.GetEntityQuery(ComponentType.ReadOnly<Game.Buildings.TelecomFacility>(), ComponentType.ReadOnly<Transform>(), ComponentType.ReadOnly<PrefabRef>(), ComponentType.Exclude<Game.Buildings.ServiceUpgrade>(), ComponentType.Exclude<Temp>(), ComponentType.Exclude<Deleted>());
        ...
        protected override void OnUpdate()
        {
            if (this.m_TerrainSystem.GetHeightData().isCreated)
            {
                ...
                TelecomCoverageJob jobData = default(TelecomCoverageJob);
        ...
        public new NativeArray<TelecomCoverage> GetMap(bool readOnly, out JobHandle dependencies)
        {
        ...
        public new CellMapData<TelecomCoverage> GetData(bool readOnly, out JobHandle dependencies)
        {
            dependencies = (readOnly ? this.m_WriteDependencies : JobHandle.CombineDependencies(this.m_ReadDependencies, this.m_WriteDependencies));
