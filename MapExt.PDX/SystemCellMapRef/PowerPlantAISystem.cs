...
{
	/// <summary>
	/// 反编译代码存在net库引用问题，暂不修补
	/// </summary>
	public class PowerPlantAISystemRe
	{
        [HarmonyPatch]
        public static class JobReplacementPatch
        {
            ...
            {
                return AccessTools.Method(typeof(PowerPlantAISystem), "OnUpdate");
            }
            ...
            static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator il)
            {
                return JobReplaceTranspilerHelper.ReplaceJobSchedule(
                    instructions,
                    oldJobType: AccessTools.Inner(typeof(PowerPlantAISystem), "PowerPlantTickJob"),
                    newJobType: typeof(PowerPlantTickJob),
                    logPrefix: "[PowerPlantAISystemJobReplacePatch] "
                );
            }
        }
		...
		private struct PowerPlantTickJob : IJobChunk
		{
			...
			public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
			{
				...
				for (int i = 0; i < chunk.Count; i++)
				{
					...
					{
						Wind wind = WindSystemRe.GetWind(nativeArray6[i].m_Position, this.m_WindMap);
						float2 = PowerPlantAISystemRe.GetWindProduction(componentData5, wind, efficiency);
					...
					{
						zero2 += PowerPlantAISystemRe.GetGroundWaterProduction(componentData8, nativeArray6[i].m_Position, efficiency, this.m_GroundWaterMap);
					}
			...
			private float2 GetWaterProduction(WaterPoweredData waterData, Game.Buildings.WaterPowered waterPowered, DynamicBuffer<Game.Net.SubNet> subNets, float efficiency)
			{
				...
				float num2 = efficiency * PowerPlantAISystemRe.GetWaterCapacity(waterPowered, waterData);
		...
		{
			float num = (float)GroundWaterSystemRe.GetGroundWater(position, groundWaterMap).m_Amount / (float)groundWaterData.m_MaximumGroundWater;
